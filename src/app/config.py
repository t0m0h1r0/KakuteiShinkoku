...
class ConfigManager:
    ...
    def _merge_config(self, file_config: Dict[str, Any]) -> None:
        ...
        if "default_exchange_rate" in file_config:
            self._config_options.exchange_rate_default = float(
                file_config["default_exchange_rate"]
            )
        ...
        if "output_files" in file_config:
            self._config_options.output_files.update(
                {
                    k: v
                    for k, v in file_config["output_files"].items()
    ...
    def _override_from_env(self) -> None:
        ...
        try:
            ...
            if color_env is not None:
                self._config_options.use_color = color_env.lower() in [
                    "true",
                    "1",
                    "yes",
                ]
    ...
    def _validate_config(self) -> None:
        ...
        for file_path in self._config_options.transaction_files:
            if not Path(file_path).exists():
                self.logger.warning(
                    f"トランザクションファイルが見つかりません: {file_path}"
                )
        ...
        # ログディレクトリの作成
        Path(self._config_options.logging_config["log_dir"]).mkdir(
    ...
    def create_logging_config(self) -> Dict[str, Any]:
        ...
        return {
            "version": 1,
            "formatters": {"detailed": {"format": self.logging_config["log_format"]}},
            "handlers": {
                "console": {
