...
class InterestProcessor(BaseProcessor[InterestTradeRecord]):
    ...
    def _process_daily_transactions(
        ...
        interest_transactions = [
            t for t in transactions if self._is_interest_transaction(t)
        ]
    ...
    def _process_transaction(self, transaction: Transaction) -> None:
        ...
        try:
            ...
            self._transaction_tracker.update_tracking(
                transaction.symbol or "GENERAL", gross_amount.usd, tax_money.usd
            )
    ...
    def _is_interest_transaction(self, transaction: Transaction) -> bool:
        """利子トランザクションの判定"""
        return (
            transaction.action_type.upper() in InterestProcessingConfig.INTEREST_ACTIONS
            and abs(transaction.amount)
            > InterestProcessingConfig.MINIMUM_TAXABLE_INTEREST
        )
