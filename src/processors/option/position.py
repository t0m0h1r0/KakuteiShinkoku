...
class OptionPosition:
    ...
    def close_position(
        ...
        if total_available < quantity:
            raise ValueError(
                f"決済に必要な数量が不足しています。必要: {quantity}, 利用可能: {total_available}"
            )
        ...
        while remaining_quantity > 0 and contracts:
            ...
            if close_quantity == contract.quantity:
                ...
                contract.fees = contract.fees * (
                    contract.quantity / (contract.quantity + close_quantity)
                )

            remaining_quantity -= close_quantity

        return {
            "realized_gain": realized_gain.quantize(
